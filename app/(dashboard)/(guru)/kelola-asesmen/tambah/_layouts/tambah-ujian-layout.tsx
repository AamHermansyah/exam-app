...
export default function TambahUjianLayout({ token }: IProps) {
      ...
      createExam(values, token)
        .then((data) => {
          if (data.status === 'success') {
            // @ts-ignore
            navigate.push(`/kelola-asesmen/edit?id=${data.data?.id}`);
          } else {
            // @ts-ignore
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <h1 className="text-lg font-medium mb-4">Tambah Asesmen</h1>
      <Form {...form}>
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 items-start gap-4">
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="col-span-2">
                <FormLabel>Judul Asesmen</FormLabel>
                <FormControl>
                  <Input placeholder="Judul Asesmen" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Durasi Asesmen (menit)</FormLabel>
                <FormControl>
                  <Input
                    inputMode="numeric"
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  <FormControl>
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder="Pilih kategori asesmen" />
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
