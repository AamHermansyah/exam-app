    ...
    <div className="max-w-3xl mx-auto py-10 space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between items-center">
        <div className="flex items-center text-sm text-muted-foreground gap-2">
          ...
          Setelah tombol &quot;Mulai Asesmen&quot; ditekan, waktu akan langsung berjalan.
        </div>
        {!!submission && (submission?.score !== null || isSubmissionExpired(submission?.expireAt)) ? (
          ...
          <Button
            ...
            onClick={() => {
              if (!!submission) navigate.push(`/asesmen/${data.id}`);
              else handleStartExam();
            }}
            disabled={loading}
          >
            {loading && <LoaderCircle className="w-4 h-4 animate-spin" />}
            {!!submission ? 'Lanjutkan' : 'Mulai Asesmen'}
          </Button>
        )}
      </div>
    </div>
